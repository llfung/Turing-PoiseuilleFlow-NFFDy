...
plot!(plt,ymesh,sol, label="Prandtl Mixing Length (given parameters)")

## Data Assimilation
@model function fit(data)
    # Scale-uniform priors
    ## (FILL IN THE BLANK) ##
    # Invoke the solver
    ## (FILL IN THE BLANK) ##
    ...
    for i in 2:length(datamesh)
        data[i] ~ ## (FILL IN THE BLANK) ##  ## Noise standard deviation is given by DNSData["U_std"][i] ##
    end
end
...
map_estimate= maximum_a_posteriori(model; lb=[logAp_lb,logκ_lb], ub=[logAp_ub, logκ_ub])
StatsBase.coeftable(map_estimate)

## Post-Processing Plots
p_result = (G=p.G,Re=p.Re,Ap=map_estimate.values[## (FILL IN THE :(Variable Name) ##],κ=map_estimate.values[## (FILL IN THE :(Variable Name) ##])
